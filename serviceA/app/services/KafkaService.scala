...
@Singleton
class KafkaService @Inject()(conf: Configuration) extends LazyLogging {
  ...
  def consumeFromKafka(): List[Call] = {
    ...
    consumer.poll(Duration.ofSeconds(1)).asScala.toList.map(_.value).map(Json.parse(_).as[Call])
  }
}
